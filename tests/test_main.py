...
@pytest.mark.asyncio
async def test_do_main_dotnet_archive_packages(mocker: MockerFixture, tmp_path: Path,
                                               mock_settings: Mock) -> None:
    cat = 'cat'
    pkg = 'pkg'
    ebuild_version = '1.0.0'
    ...
    ebuild_path.write_text('SHA="1234567"\n', encoding='utf-8')
    mock_settings.auto_update_flag = True
    mock_settings.dotnet_projects = {cp: 'proj.csproj'}
    mock_settings.dotnet_packages = {cp: True}
    mocker.patch('livecheck.main.check_dotnet_requirements', return_value=True)
    mocker.patch('livecheck.main.get_old_sha', return_value='1234567')
    mocker.patch('livecheck.main.replace_date_in_ebuild', side_effect=lambda v, _, __: v)
    mocker.patch('livecheck.main.remove_leading_zeros', side_effect=lambda v: v)
    ...
    mock_async_proc.wait = mocker.AsyncMock(return_value=0)
    mocker.patch('livecheck.main.asyncio.create_subprocess_exec', return_value=mock_async_proc)
    mocker.patch('livecheck.main.update_dotnet_ebuild')
    mocker.patch('livecheck.main.remove_dotnet_url', side_effect=lambda c: c)
    mock_archive = mocker.patch('livecheck.main.update_dotnet_archive_ebuild')
    await do_main(cat=cat,
                  ebuild_version=ebuild_version,
                  hash_date=hash_date,
                  hook_dir=hook_dir,
...
@pytest.mark.asyncio
async def test_do_main_dotnet_recovers_on_exception(mocker: MockerFixture, tmp_path: Path,
                                                    mock_settings: Mock) -> None:
    cat = 'cat'
    pkg = 'pkg'
    ebuild_version = '1.0.0'
    ...
    mock_async_proc.wait = mocker.AsyncMock(return_value=0)
    mocker.patch('livecheck.main.asyncio.create_subprocess_exec', return_value=mock_async_proc)
    mocker.patch('livecheck.main.update_dotnet_ebuild', side_effect=RuntimeError('boom'))
    mock_jetbrains = mocker.patch('livecheck.main.update_jetbrains_ebuild')
    mock_settings.jetbrains_packages = {cp: True}
    await do_main(cat=cat,
                  ebuild_version=ebuild_version,
                  hash_date=hash_date,
                  hook_dir=hook_dir,
                  ...
                  top_hash=top_hash,
                  url=url)
    mock_jetbrains.assert_not_called()
...
@pytest.mark.asyncio
async def test_parse_url_dispatches_to_nuget(mocker: MockerFixture) -> None:
    mocker.patch('livecheck.main.is_pypi', return_value=False)
    mocker.patch('livecheck.main.is_nuget', return_value=True)
    mocker.patch('livecheck.main.urlparse',
                 return_value=mocker.Mock(hostname='api.nuget.org', path='/v3-flatcontainer/foo'))
    nuget_call = mocker.patch('livecheck.main.get_latest_nuget_package',
                              return_value=('1.2.3', 'https://api.nuget.org/.../foo.1.2.3.nupkg'))
    settings = mocker.Mock()
    last_version, top_hash, hash_date, url = await parse_url(
        'https://api.nuget.org/v3-flatcontainer/foo/index.json',
        'cat/pkg-1.0.0',
        settings,
        force_sha=False)
    assert last_version == '1.2.3'
    assert not top_hash
    assert not hash_date
    assert url == 'https://api.nuget.org/.../foo.1.2.3.nupkg'
    nuget_call.assert_awaited_once()
...
@pytest.mark.parametrize(
    ...
    [('github', 'get_latest_github_metadata', ('latest_version', 'top_hash'),
     ...
     ('nuget', 'get_latest_nuget_metadata', ('latest_version', 'url'),
      ('latest_version', '', '', 'url'))])
...
def test_main_rejects_dist_github_flags_used_alone(mocker: MockerFixture, runner: CliRunner,
                                                   tmp_path: Path) -> None:
    mocker.patch('livecheck.main.chdir')
    mocker.patch('livecheck.main.setup_logging')
    mocker.patch('livecheck.main.gather_settings')
    ...
    mocker.patch('livecheck.main.os.access', return_value=True)
    result = runner.invoke(
        main, ['--dist-github-repository', 'Tatsh/livecheck', '--working-dir',
               str(tmp_path)])
    assert result.exit_code != 0
    assert '`--dist-github-repository` and `--dist-github-release` must be used together.' in (
        result.output)
...
async def test_get_props_type_location_checksum_calls_get_latest_location_checksum_package(
    ...
    mock_settings2.request_headers = {'cat/pkg': {'Referer': 'https://example.com'}}
    mock_settings2.request_params = {'cat/pkg': {'agree': 'Yes'}}
    mocker.patch('livecheck.main.get_highest_matches', return_value=['cat/pkg-1.0.0'])
    mocker.patch('livecheck.main.catpkg_catpkgsplit',
                 return_value=('cat/pkg', 'cat', 'pkg', '1.0.0'))
    mocker.patch('livecheck.main.get_first_src_uri',
...
def test_get_old_sha_bare_hex_string(tmp_path: Path) -> None:
    ...
    ebuild_path.write_text(content, encoding='utf-8')
    result = get_old_sha(ebuild_path, '')
    assert result == 'fb0db0841e097de4ed819066a45933748172e02a'


def test_get_old_sha_named_preferred_over_bare(tmp_path: Path) -> None:
    content = ('EAPI=8\n'
               'S="${WORKDIR}/tempy-fb0db0841e097de4ed819066a45933748172e02a"\n'
...
async def test_process_submodules_3tuple_parent_not_ok(mocker: MockerFixture) -> None:
    mocker.patch('livecheck.main.SUBMODULES',
                 {'foo': [('nested/path', 'NESTED_SHA', 'parent/submod')]})
    mock_get_content = mocker.patch('livecheck.main.get_content')
    mock_get_content.return_value = mocker.Mock(ok=False)
    contents = 'NESTED_SHA="old_sha"\n'
    repo_uri = 'https://api.github.com/repos/org/repo'
    mocker.patch('livecheck.main.urlparse', return_value=mocker.Mock(path='/org/repo'))
    result = await process_submodules('foo', 'main', contents, repo_uri)
...
async def test_process_submodules_3tuple_no_submodule_git_url(mocker: MockerFixture) -> None:
    mocker.patch('livecheck.main.SUBMODULES',
                 {'foo': [('nested/path', 'NESTED_SHA', 'parent/submod')]})
    mock_get_content = mocker.patch('livecheck.main.get_content')
    parent_response = mocker.Mock(ok=True, json=lambda: {'sha': 'parentsha123'})
    mock_get_content.return_value = parent_response
    contents = 'NESTED_SHA="old_sha"\n'
    repo_uri = 'https://api.github.com/repos/org/repo'
    mocker.patch('livecheck.main.urlparse', return_value=mocker.Mock(path='/org/repo'))
    result = await process_submodules('foo', 'main', contents, repo_uri)
...
async def test_do_main_sha_40_char_replacement(mocker: MockerFixture, tmp_path: Path,
    ...
    ebuild_path = tmp_path / f'{cat}/{pkg}/{pkg}-1.0.0.ebuild'
    ebuild_path.parent.mkdir(parents=True)
    ebuild_path.write_text(f'SHA="{old_sha}"\nSHORT={old_sha[:7]}\n', encoding='utf-8')
    mock_settings.sha_sources = {}
    mock_settings.auto_update_flag = True
    mocker.patch('livecheck.main.get_old_sha', return_value=old_sha)
    mocker.patch('livecheck.main.replace_date_in_ebuild', side_effect=lambda v, _, __: v)
...
async def test_do_main_pythonhosted_url_replacement(mocker: MockerFixture, tmp_path: Path,
    ...
    search_dir = tmp_path
    cp = f'{cat}/{pkg}'
    old_hash_path = '/packages/aa/bb/cccccccc'
    new_hash_path = '/packages/dd/ee/ffffffff'
    url = f'https://files.pythonhosted.org{new_hash_path}/pkg-1.0.1.tar.gz'
    ebuild_path = tmp_path / f'{cat}/{pkg}/{pkg}-1.0.0.ebuild'
    ebuild_path.parent.mkdir(parents=True)
    ebuild_path.write_text(
        f'SRC_URI="https://files.pythonhosted.org{old_hash_path}/pkg-1.0.0.tar.gz"\n',
        encoding='utf-8')
    mock_settings.sha_sources = {}
    mock_settings.auto_update_flag = True
    mocker.patch('livecheck.main.get_old_sha', return_value='')
...
async def test_do_main_pythonhosted_url_no_match_in_content(mocker: MockerFixture, tmp_path: Path,
    ...
    search_dir = tmp_path
    cp = f'{cat}/{pkg}'
    url = 'https://files.pythonhosted.org/packages/dd/ee/ffffffff/pkg-1.0.1.tar.gz'
    ebuild_path = tmp_path / f'{cat}/{pkg}/{pkg}-1.0.0.ebuild'
    ebuild_path.parent.mkdir(parents=True)
    ebuild_path.write_text('SRC_URI="https://example.com/pkg-1.0.0.tar.gz"\n', encoding='utf-8')
    mock_settings.sha_sources = {}
    mock_settings.auto_update_flag = True
    mocker.patch('livecheck.main.get_old_sha', return_value='')
    mocker.patch('livecheck.main.replace_date_in_ebuild', side_effect=lambda v, _, __: v)
